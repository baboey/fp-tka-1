...
def write_log(action: str, collection: str, target_id: str, detail: dict = None):
    # Logging hanya jika ada user_id aktif
...
def get_auth_user():
    """Mendapatkan data user dari JWT token secara opsional."""
...
def login_required(f):
    ...
    def wrapper(*args, **kwargs):
        user_id, role = get_auth_user()
        if not user_id:
            return err("Token tidak ditemukan atau tidak valid", 401)
        g.user_id = user_id
        g.role    = role
...
# ═══════════════════════════════════════════
# ORDER endpoints (JWT Authenticated)
# ═══════════════════════════════════════════
...
def list_orders():
    # Cek token opsional untuk mengakomodasi request unauthenticated dari frontend simpel
    user_id, role = get_auth_user()
    if user_id:
        g.user_id = user_id
        g.role    = role
        ...
        # Jika unauthenticated, tampilkan semua (seperti API simpel)
        query = {}
    ...
    if status:
        query["status"] = status
    if city and user_id and g.role == "admin":
    ...
    docs   = list(orders_col.find(query)
                  .sort("created_at", DESCENDING).skip((page-1)*limit).limit(limit))

    # Frontend simpel mengharapkan array mentah secara langsung
    if not user_id:
        return jsonify([serialize(d) for d in docs]), 200

    return jsonify({"page": page, "limit": limit, "total": total,
                    "total_pages": -(-total // limit), "data": [serialize(d) for d in docs]}), 200
...
# ═══════════════════════════════════════════
# COMPATIBILITY endpoints (Unauthenticated)
# Untuk melayani index.html simpel bawaan
# ═══════════════════════════════════════════
...
def create_order_simple():
    try:
        ...
        if not product or qty < 1 or price < 0:
        ...
        doc = {
            ...
            "payment_status":  "unpaid",
            "notes":           "Dibuat via web frontend simpel",
            "created_at":      now,
            "updated_at":      now,
        }
        result = orders_col.insert_one(doc)
...
def index_health():
    # Endpoint default untuk check database
    try:
        client.admin.command('ismaster')
    except Exception:
        return jsonify({"message": "Database not available", "success": False}), 501
    return jsonify({"message": "MongoDB is connected to backend", "success": True}), 200
